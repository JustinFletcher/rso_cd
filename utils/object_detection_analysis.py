...
class ClassificationAnalysis(ABC):
    def __init__(self,
        ...
        for image_number, image_name in enumerate(self.truth_boxes.keys()):
            ...
            analyses = self.analyse_element(image_name)
...
class MulticlassClassificationAnalysis(ObjectDetectionAnalysis):
    def __init__(self,
                 truth_labels,
                 inferred_labels,
                 confidence_thresholds=None):
    ...
    def _compute_classification_counts(self,
        ...
        # Iterate over each class for this element.
        for class_number in len(truth_labels):

            # Get the truth label for this class
            class_truth = truth_labels[class_number]

            print("Assume this should be 0 or 1: " + str(class_truth))
            ...
            if class_truth == 1.0 and class_inferred == 1.0:
                ...
                counts_dict["class_" + str(class_number) + "_tp"] = 1

            else:

                counts_dict["class_" + str(class_number) + "_tp"] = 0

            if class_truth == 0.0 and class_inferred == 0.0:

                counts_dict["class_" + str(class_number) + "_tn"] = 1

            else:

                counts_dict["class_" + str(class_number) + "_tn"] = 0

            if class_truth == 0.0 and class_inferred == 1.0:

                counts_dict["class_" + str(class_number) + "_fp"] = 1

            else:

                counts_dict["class_" + str(class_number) + "_fp"] = 0

            if class_truth == 1.0 and class_inferred == 0.0:

                counts_dict["class_" + str(class_number) + "_fn"] = 1

            else:

                counts_dict["class_" + str(class_number) + "_fn"] = 0
